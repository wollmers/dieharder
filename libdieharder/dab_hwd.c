 ...
 *
 *  From http://xoshiro.di.unimi.it/hwd.php
 */
...
#define HWD_BITS (SIZEOF_SIZE_T * 8)
...
static inline uint32_t scan_batch(uint32_t sig, int64_t n, uint32_t *ts) {
    ...
    if (gsl_rng_max(rng) <= UINT32_MAX) {
        const uint32_t w = gsl_rng_get(rng);
        if (ts) {
        ...
        for (int64_t i = 0; i < n; i++) {
            const uint64_t w64 = gsl_rng_get(rng);
            uint32_t w32 = w64 >> 32;
            if (ts) {
...
static inline uint32_t scan_batch(uint32_t sig, int64_t n, uint64_t *ts) {
    ...
    if (gsl_rng_max(rng) <= UINT32_MAX) {
        ...
        for (int64_t i = 0; i < n; i++) {
            const uint64_t w = gsl_rng_get(rng);
...
static double compute_pvalue(const bool trans) {
    ...
    MYDEBUG(D_DAB_HWD){
       printf("# dab_hwd(): bits per word = %d (analyzing %s); min category p-value = %.3g\n", HWD_BITS,
              trans ? "transitions" : "bits", overall_pvalue);
    }
...
static double analyze(int64_t pos, bool trans, bool final) {
    ...
    if (pos < 2 * pow(2.0/(1.0 - P), DIM))
        printf("# dab_hwd(): WARNING: p-values are unreliable, you have to wait (insufficient data for meaningful answer)\n");

    const double pvalue = compute_pvalue(trans);
    const time_t tm = time(0);

    MYDEBUG(D_DAB_HWD){
        printf("# dab_hwd(): processed %.3g bytes in %.3g seconds (%.4g GB/s, %.4g TB/h). %s\n",
...
static double run_test(const int64_t n, const bool trans, const bool progress) {
    ...
    return analyze(pos, trans, true);
}

int dab_hwd(Test **test, unsigned int irun)
{
    unsigned int n = (ntuple == 0) ? 128 : ntuple;
    unsigned int t = (tsamples == 0) ? 100000000 : tsamples;
    test[0]->ntuple = n;
    test[0]->pvalues[irun] = run_test(t, false, verbose);
    return 0;
}
...
int main(int argc, char **argv) {
...
#ifdef HWD_MMAP
    MYDEBUG(D_DAB_HWD){
        printf("# dab_hwd(): Allocating memory via mmap()...");
    }
    ...
    cs = mmap((void *)(0x0UL), (SIZE + 1) * sizeof *cs + SIZE * sizeof *norm + SIZE * sizeof *count_sum,
              PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | (30 << MAP_HUGE_SHIFT),
              0, 0);
    if (cs == MAP_FAILED) {
